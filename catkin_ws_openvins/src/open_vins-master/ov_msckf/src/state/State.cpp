...
void State::initialize_variables() {
    ...
    // Append the imu to the state and covariance                       // 将 IMU 状态放入状态向量中，并开始维护协方差
    _imu = new IMU();
    _imu->set_local_id(current_id);                                     // 设置 IMU 状态在协方差矩阵中的位置
    ...
    if (_options.do_calib_camera_timeoffset) {
        ...
        current_id += _calib_dt_CAMtoIMU->size();               // size 返回的是自由度的维度
    }

...
namespace ov_msckf {
    ...
    public:
        ...
        struct NoiseManager {
            ...
            double sigma_w_2;                             // cov_ng

            /// Gyro random walk
            double sigma_wb;                              // bg

            /// Gyro random walk covariance
            double sigma_wb_2;                            // cov_bg

            /// Accel white noise
            double sigma_a;                               // na

            /// Accel white noise covariance
            double sigma_a_2;                             // cov_na

            /// Accel random walk
            double sigma_ab;                              // ba

            /// Accel random walk covariance
            double sigma_ab_2;                            // cov_ba
         ...
         */
        void feed_imu(double timestamp, Eigen::Vector3d wm, Eigen::Vector3d am) {           // feed_imu 存储IMU数据
    ...
    protected:
        ...
        NoiseManager _noises;                                             // 噪声管理

        /// Our history of IMU messages (time, angular, linear)
        std::vector<IMUDATA> imu_data;                                    // imu数据

        /// Gravity vector
        Eigen::Matrix<double, 3, 1> _gravity;                             // 重力加速度

...
namespace ov_msckf {
    ...
    protected:
        ...
        State* state;                          // vio 的configurator 11 个参数

        /// Propagator of our state
        Propagator* propagator;                // 传播器

        /// Boolean if we should do stereo tracking or if false do binocular
        bool use_stereo = true;               // 单目还是双目

        /// Our sparse feature tracker (klt or descriptor)
        TrackBase* trackFEATS = nullptr;      // track跟踪器

        /// Our aruoc tracker
        TrackBase* trackARUCO = nullptr;      // aruoc 跟踪器

        /// State initializer
        InertialInitializer* initializer;     // 状态初始化器

        /// Boolean if we are initialized or not
        bool is_initialized_vio = false;     // 是否成功初始化flag

        /// Our MSCKF feature updater
        UpdaterMSCKF* updaterMSCKF;          // msckf updater

        /// Our MSCKF feature updater
        UpdaterSLAM* updaterSLAM;            // slam updater
        ...
        double dt_statupdelay;                           // 插入第一个slam feature的时间延迟
        double startup_time = -1;                        // 整个系统初始化完成之后的第一个Image的时间戳

        // Camera intrinsics that we will load in    相机参数

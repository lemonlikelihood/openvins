...
int main(int argc, char** argv)
{
    ...
    sys = new VioManager(nh);                                                                // 1. 新建一个VIO管理器
    viz = new RosVisualizer(nh, sys);                                                        // 2. 新建一个可视化窗口
    ...
    nh.param<std::string>("topic_camera0", topic_camera0, "/cam0/image_raw");                  //设置参数服务器默认值，左右图像topic
    ...
    nh.param<std::string>("path_bag", path_to_bag, "/home/patrick/datasets/eth/V1_01_easy.bag");  // 设置默认bag包路径，参数服务器上找不到会用默认值代替
    ...
    view_full.addQuery(bag);                                       // 查询整个bag时间戳
    ros::Time time_init = view_full.getBeginTime();
    time_init += ros::Duration(bag_start);
    ros::Time time_finish = (bag_durr < 0)? view_full.getEndTime() : time_init + ros::Duration(bag_durr);
    ROS_INFO("time start = %.6f", time_init.toSec());
    ROS_INFO("time end   = %.6f", time_finish.toSec());
    view.addQuery(bag, time_init, time_finish);                   // 重新设置bag查询时间戳

    // Check to make sure we have data to play
    if (view.size() == 0) {                                       // 检查 rosbag 时间线
    ...
    bool has_left = false;                                         // 是否接受到左图像数据
    bool has_right = false;                                        // 是否接受到右图像数据
    cv::Mat img0, img1;                                            // 左图像，右图像
    ...
    for (const rosbag::MessageInstance& m : view) {
        ...
        if (s2 != NULL && m.getTopic() == topic_imu) {
            ...
            sys->feed_measurement_imu(timem, wm, am);                            //  IMU处理的主回调函数
        }
        ...
        if(max_cameras==1 && has_left) {
            ...
            if(!gt_states.empty() && !sys->intialized() && DatasetReader::get_gt_state(time_buffer,imustate,gt_states)) {
                ...
                sys->initialize_with_gt(imustate);                                         // 主初始化回调函数
            } else if(gt_states.empty() || sys->intialized()) {                            // Image 处理的主回调函数
                sys->feed_measurement_monocular(time_buffer, img0_buffer, 0);              // feed_measurement_monocular cam_id 表明img来自哪个相机
            }
            // visualize
            viz->visualize();
            // reset bools

...
namespace ov_core {
     ...
     */
    class InertialInitializer {                            // imu 初始化器，静止初始化
    ...
    public:
         ...
         */
        struct IMUDATA {                                 // imu data struct
    ...
    protected:
        ...
        /// Gravity vector
        Eigen::Matrix<double,3,1> _gravity;                               // 重力向量

        /// Amount of time we will initialize over (seconds)
        double _window_length;                                            // 静止时长

        /// Variance threshold on our acceleration to be classified as moving
        double _imu_excite_threshold;                                     // 静止协方差大小阈值

        /// Our history of IMU messages (time, angular, linear)
        std::vector<IMUDATA> imu_data;                                    // 窗口内的IMU数据

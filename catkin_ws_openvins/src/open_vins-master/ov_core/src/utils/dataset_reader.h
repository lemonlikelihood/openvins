...
namespace ov_core {
    ...
    class DatasetReader {
        ...
        /// 读取 groundtruth 数据按时间戳存入 gt_states中
    ...
    public:
        ...
        static bool get_gt_state(double timestep, Eigen::Matrix<double,17,1> &imustate, std::map<double, Eigen::Matrix<double,17,1>>& gt_states) {
            ...
            imustate(1, 0) = state(5, 0); //quat   // q.x
            imustate(2, 0) = state(6, 0);          // q.y
            imustate(3, 0) = state(7, 0);          // q.z
            imustate(4, 0) = state(4, 0);          // q.w
            imustate(5, 0) = state(1, 0); //pos    // p.x
            imustate(6, 0) = state(2, 0);          // p.y
            imustate(7, 0) = state(3, 0);          // p.z
            imustate(8, 0) = state(8, 0); //vel    // v.x
            imustate(9, 0) = state(9, 0);          // v.y
            imustate(10, 0) = state(10, 0);        // v.z
            imustate(11, 0) = state(11, 0); //bg   // bg_x
            imustate(12, 0) = state(12, 0);        // bg_y
            imustate(13, 0) = state(13, 0);        // bg_z
            imustate(14, 0) = state(14, 0); //ba   // ba_x
            imustate(15, 0) = state(15, 0);        // ba_y
            imustate(16, 0) = state(16, 0);        // ba_z

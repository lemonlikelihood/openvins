...
namespace ov_core {
    ...
    public:
        ...
        static void perform_griding(const cv::Mat &img, std::vector<cv::KeyPoint> &pts, int num_features,
            ...
            auto num_features_grid = (int) (num_features / (grid_x * grid_y)) + 1;        // 将所有的特征点平均分配到所有网格中，每一个grid中至少需要的特征点数量

            // Parallelize our 2d grid extraction!!
            int ct_cols = std::floor(img.cols/size_x);                                    // 实际的列数
            int ct_rows = std::floor(img.rows/size_y);                                    // 实际的行数
            std::vector<std::vector<cv::KeyPoint>> collection(ct_cols*ct_rows);           // 每一个网格里面都有一个vector
            parallel_for_(cv::Range(0, ct_cols*ct_rows), [&](const cv::Range& range){
                for (int r = range.start; r < range.end; r++) {
                    ...
                    cv::FAST(img(img_roi), pts_new, threshold, nonmaxSuppression);         // 调用opencv提取fast角点

                    // Now lets get the top number from this
                    std::sort(pts_new.begin(), pts_new.end(), Grider_FAST::compare_response);  // 按照响应值进行特征点排序
                    ...
                    for (size_t i = 0; i < (size_t) num_features_grid && i < pts_new.size(); i++) {        // 按指定个数的特征点加入vector中

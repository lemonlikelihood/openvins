...
namespace ov_core {
    ...
    public:
        ...
        void set_calibration(std::map<size_t,Eigen::VectorXd> camera_calib,
            ...
            if(mtx_feeds.empty() || camera_k_OPENCV.empty() || camera_k_OPENCV.empty() || this->camera_fisheye.empty()) {
                ...
                for (auto const &cam : camera_calib) {
                    ...
                    assert(cam.second.rows()==8);               // [fx,fy,cx,cy,d1,d2,d3,d4]
                    // Camera matrix
                    cv::Matx33d tempK;                          // cv::Matx33d
                    ...
                    cv::Vec4d tempD;                         // cv::Vec4d d1,d2,d3,d4
        ...
        std::map<size_t, cv::Matx33d> get_camera_k_OPENCV() {
            return camera_k_OPENCV;
        }

        std::map<size_t, cv::Vec4d> get_camera_d_OPENCV(){
        ...
        cv::Point2f undistort_point(cv::Point2f pt_in, size_t cam_id) {
            ...
            if (this->camera_fisheye.at(cam_id)) {
                return undistort_point_fisheye(pt_in, camK, camD);   // 如果是鱼眼相机，采用鱼眼相机去畸变模型
            }
            return undistort_point_brown(pt_in, camK, camD);        // 否则采用brown模型去畸变，针孔模型
        }
    ...
    protected:
        ...
        cv::Point2f undistort_point_fisheye(cv::Point2f pt_in, cv::Matx33d &camK, cv::Vec4d &camD) {
            // Convert point to opencv format          输入图像坐标系坐标，返回去畸变后的图像坐标系坐标
            cv::Mat mat(1, 2, CV_32F);
            mat.at<float>(0, 0) = pt_in.x;
            mat.at<float>(0, 1) = pt_in.y;
            mat = mat.reshape(2); // Nx1, 2-channel
        ...
        FeatureDatabase *database;                       // 当前跟踪到的特征点的database

        /// If we are a fisheye model or not
        std::map<size_t, bool> camera_fisheye;          // 相机组是否为鱼眼相机

        /// Camera intrinsics in OpenCV format
        std::map<size_t, cv::Matx33d> camera_k_OPENCV;  // 相机组对应的内参数

        /// Camera distortion in OpenCV format
        std::map<size_t, cv::Vec4d> camera_d_OPENCV;    // 相机组对应的畸变系数

        /// Number of features we should try to track frame to frame
        int num_features;                              // 需要track的特征点数目
        ...
        std::unordered_map<size_t, std::vector<cv::KeyPoint>> pts_last;  // size_t 表示camera_id 上一帧track到的关键点数据,图像坐标系
